...
// 		exit;
		...
		foreach ($topcate as $k => $v){
			...
			$m=D('News');
			$topcate[$k]['news']=$m->where($where)->where("news_dell=0")->relation(true)->select();
			$result=$topcate[$k]['news'];
			$result = $m->parseFieldsMap($result);
...
// 			exit;
			...
			$Page = new \Think\Page($count,C('PAGE_ARTICLE__HOME'));// 实例化分页类 传入总记录数和每页显示的记录数(25)
    		...
    		$where=array('nv_id'=>array('IN', $cids));//查询新闻表nv_id字段和$cids相等时的数据
    		$m=D('News');
    		$field='g.id,g.news_title,g.news_description,g.news_content,g.news_hits,g.news_addtime,g.news_updatetime,g.news_author,g.news_sort,g.news_pic,i.attr_color,i.attr_name,c.column_name';
